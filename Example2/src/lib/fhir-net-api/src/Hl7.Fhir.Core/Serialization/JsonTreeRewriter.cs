...
namespace Hl7.Fhir.Serialization
{
    internal static class JsonTreeRewriter
    {
        ...
        private static IEnumerable<JProperty> expandPrimitiveChildren(IEnumerable<JProperty> children,
            string parentName)
        {
            var inResource = children.Any(prop => prop.Name == JsonDomFhirReader.RESOURCETYPE_MEMBER_NAME);

            foreach (var child in children)
                if (child.Value.Type == JTokenType.Null)
                    yield return child;
                else if (child.Value is JValue && !isTruePrimitive(child, parentName, inResource))
                    yield return new JProperty(child.Name,
                        new JObject(new JProperty(PRIMITIVE_PROP_NAME, child.Value)));
                else
                    yield return child;
        }
        ...
        private static bool isTruePrimitive(JProperty property, string parentName, bool inResource)
        {
            return property.Value is JValue &&
                   ...
                   property.Name == SPEC_CHILD_URL && parentName != null &&
                   parentName.StartsWith(SPEC_PARENT_EXTENSION) || // url attr of extension
                   property.Name == SPEC_CHILD_URL && parentName != null &&
                   parentName.StartsWith(SPEC_PARENT_MODIFIEREXTENSION); // contentType attr of Binary resource
        }
        ...
        {
            if (prop == null && appendix == null)
            {
                throw new FormatException("property value and appendix (_property) cannot both be null");
            }


            if (appendix != null && appendix.Value.Type == JTokenType.Null)
            {
                appendix = null;
            }


            if (appendix == null && prop != null)
            {
                return new JProperty(prop.Name, prop.Value.Type == JTokenType.Null ? new JObject() : prop.Value);
            }
            ...
            {
                return new JProperty(baseChildName(appendix.Name), appendix.Value);
            }
            ...
            if (to == null || from == null)
            {
                throw new InvalidOperationException(
                    "Internal logic error: trying to merge base/appendix properties which are not JObjects");
            }
