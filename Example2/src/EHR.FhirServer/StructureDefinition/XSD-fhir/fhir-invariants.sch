...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Organization">
      <sch:assert test="count(f:identifier | f:name) &gt; 0">org-1: Организация ДОЛЖНА иметь по крайней мере название (элемент name) или идентификатор (элемент identifier), и, возможно, больше чем один</sch:assert>
    </sch:rule>
    ...
    <sch:rule context="f:Organization/f:telecom">
      <sch:assert test="count(f:use[@value='home']) = 0">org-3: У элемента telecom организации никогда не используется тип применения 'home'</sch:assert>
    </sch:rule>
    <sch:rule context="f:Organization/f:telecom">
    ...
    <sch:rule context="f:Organization/f:address">
      <sch:assert test="count(f:use[@value='home']) = 0">org-2: У элемента address организации никогда не используется тип применения 'home'</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:ValueSet">
      <sch:assert test="not(f:define/f:system/@value = f:identifier/@value)">vsd-7: Указанная кодовая система (если задана) ДОЛЖНА иметь идентификатор, отличающийся от идентификатора самого набора значений</sch:assert>
      <sch:assert test="exists(f:define) or exists(f:compose) or exists(f:expansion)">vsd-5: Набор значений ДОЛЖЕН содержать либо define, либо compose, либо expansion элемент</sch:assert>
      <sch:assert test="not(exists(f:compose)) or (count(f:compose/f:import)!=1 or exists(f:compose/f:include) or exists(f:compose/f:exclude) or exists(f:define))">vsd-2: Набор значений с одним импортом ДОЛЖЕН также иметь включение и/или исключение, если только этот набор значений не определяет собственные коды</sch:assert>
    </sch:rule>
    ...
    <sch:rule context="f:ValueSet/f:define">
      <sch:assert test="count(descendant::f:concept)=count(distinct-values(descendant::f:concept/f:code/@value))">vsd-8: Коды должны быть уникальными</sch:assert>
      <sch:assert test="count(distinct-values(descendant::f:concept/f:code/@value))=count(descendant::f:concept)">vsd-3: В пределах определения кодовой системы все коды ДОЛЖНЫ быть уникальными</sch:assert>
    </sch:rule>
    <sch:rule context="f:ValueSet/f:define/f:concept/f:designation/f:use">
      <sch:assert test="not (exists(f:valueSet) and exists(f:code)) or exists(f:system)">cod-1: Если указан valueSet, то также необходимо указать и URI системы</sch:assert>
    </sch:rule>
    <sch:rule context="f:ValueSet/f:compose">
      <sch:assert test="exists(f:include) or exists(f:import)">vsd-1: В композицию набора значений ДОЛЖНО входить включение или импорт</sch:assert>
    </sch:rule>
    <sch:rule context="f:ValueSet/f:expansion/f:contains">
      <sch:assert test="exists(f:code) or (f:display/@value = 'true')">vsd-9: Должен иметь код, если не абстрактный</sch:assert>
      <sch:assert test="exists(f:code) or exists(f:display)">vsd-6: Должен иметь элемент code или display</sch:assert>
      <sch:assert test="exists(f:system) or not(exists(f:code))">vsd-10: Должен иметь систему, если код указан</sch:assert>
    </sch:rule>
    </sch:pattern>
  <sch:pattern>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:MedicationDispense">
      <sch:assert test="not(exists(f:whenHandedOver/@value)) or not(exists(f:whenPrepared/@value)) or ( f:whenHandedOver/@value &gt;= f:whenPrepared/@value)">mdd-1: Дата whenHandedOver не может быть меньше даты whenPrepared</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Composition/f:section">
      <sch:assert test="(exists(f:content) and not(exists(f:section))) or  (exists(f:section) and not(exists(f:content)))">cmp-1: Раздел должен иметь либо подразделы, либо содержимое</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Conformance">
      ...
      <sch:assert test="count(f:software | f:implementation | f:description) &gt; 0">cnf-2: Заявление о соответствии ДОЛЖНО иметь хотя бы один из [элементов] description, software или implementation</sch:assert>
      <sch:assert test="count(f:messaging)&lt;=1 or not(f:messaging[not(f:endpoint)])">cnf-4: Если имеется больше одного элемента messaging, точка взаимодействия должна быть указана для каждого элемента</sch:assert>
      <sch:assert test="count(f:messaging/f:endpoint)=count(distinct-values(f:messaging/f:endpoint/@value))">cnf-5: Набор точек взаимодействия, перечисленный для messaging, должен быть уникальным</sch:assert>
      <sch:assert test="count(f:document[f:mode='producer'])=count(distinct-values(f:document[f:mode='producer']/f:profile/@value)) and count(f:document[f:mode='consumer'])=count(distinct-values(f:document[f:mode='consumer']/f:profile/@value))">cnf-7: Набор документов должен иметь уникальную комбинацию profile &amp; mode</sch:assert>
      <sch:assert test="count(f:rest)=count(distinct-values(f:rest/f:mode/@value))">cnf-8: Может быть только одно REST-объявление для одного режима</sch:assert>
    </sch:rule>
    ...
    <sch:rule context="f:Conformance/f:rest">
      <sch:assert test="count(f:query)=count(distinct-values(f:query/f:name/@value))">cnf-10: Данный запрос может быть описан для каждого RESTful-режима только один раз</sch:assert>
      <sch:assert test="count(f:resource)=count(distinct-values(f:resource/f:type/@value))">cnf-9: Данный ресурс может быть описан для каждого RESTful-режима только один раз</sch:assert>
    </sch:rule>
    ...
    <sch:rule context="f:Conformance/f:rest/f:resource">
      <sch:assert test="count(f:searchParam)=count(distinct-values(f:searchParam/f:name/@value))">cnf-12: Имена параметров поиска должны быть уникальными в контексте ресурса</sch:assert>
      <sch:assert test="count(f:operation)=count(distinct-values(f:operation/f:code/@value))">cnf-11: Коды операций должны быть уникальными в контексте ресурса</sch:assert>
    </sch:rule>
    ...
    <sch:rule context="f:Conformance/f:messaging">
      <sch:assert test="exists(f:endpoint) = exists(parent::f:Conformance/f:implementation)">cnf-3: Точка взаимодействия обмена сообщениями необходима (и разрешена только тогда), когда это implementation statement</sch:assert>
      <sch:assert test="count(f:event[f:mode='sender'])=count(distinct-values(f:event[f:mode='sender']/f:code/@value)) and count(f:event[f:mode='receiver'])=count(distinct-values(f:event[f:mode='receiver']/f:code/@value))">cnf-6: Набор событий для точки взаимодействия messaging должен иметь уникальную комбинацию code &amp; mode</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:NamingSystem">
      <sch:assert test="not(exists(for $type in distinct-values(f:uniqueId/f:type) return if (count(f:uniqueId[f:type/@value=$type and f:primary/@value='true'])&gt;1) then $type else ()))">nsd-2: Не может иметь больше одного предпочтительного идентификатора для типа</sch:assert>
      <sch:assert test="not(f:replacedBy) or f:status/@value='retired'">nsd-3: Может иметь только replacedBy, если namingsystem удалена</sch:assert>
      <sch:assert test="not(f:type/@value='root' and f:uniqueId/f:type=('uuid', 'ruid'))">nsd-1: Корневые системы не могут иметь UUID- или SID-идентификаторов</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Media">
      <sch:assert test="not(f:type/@value='audio') or not(f:height)">mda-1: Высота используется только для фото или видео</sch:assert>
      <sch:assert test="not(f:type/@value='audio') or not(f:width)">mda-2: Ширина используется только для фото или видео</sch:assert>
      <sch:assert test="not(f:type/@value='photo') or not(f:length)">mda-4: Длина используется только для аудио или видео</sch:assert>
      <sch:assert test="(f:type/@value='photo') or not(f:frames)">mda-3: Кадры используются только для фото</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Order/f:when">
      <sch:assert test="exists(f:code) != exists(f:schedule)">ord-1: Указывайте либо код, либо расписание, но не оба элемента</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:QuestionnaireAnswers/f:group">
      <sch:assert test="not(exists(f:group) and exists(f:question))">qan-1: Группы могут содержать либо вопросы, либо группы, но не и то, и другое</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:MedicationAdministration/f:reasonNotGiven">
      <sch:assert test="not(exists(f:reasonNotGiven) and f:wasNotGiven/@value='false')">mad-2: Причину reasonNotGiven разрешается указывать только когда wasNotGiven имеет значение true</sch:assert>
    </sch:rule>
    <sch:rule context="f:MedicationAdministration/f:reasonNotGiven">
      <sch:assert test="count(f:coding[f:primary/@value='true'])&lt;=1">ccc-2: Только один кодинг из набора может быть выбран непосредственно пользователем</sch:assert>
    </sch:rule>
    ...
    <sch:rule context="f:MedicationAdministration/f:reasonGiven">
      <sch:assert test="not(exists(f:reasonGiven) and f:wasNotGiven/@value='true')">mad-3: Причину reasonGiven разрешается указывать только когда wasNotGiven имеет значение false</sch:assert>
    </sch:rule>
    <sch:rule context="f:MedicationAdministration/f:reasonGiven">
      <sch:assert test="count(f:coding[f:primary/@value='true'])&lt;=1">ccc-2: Только один кодинг из набора может быть выбран непосредственно пользователем</sch:assert>
    </sch:rule>
    ...
    <sch:rule context="f:MedicationAdministration/f:dosage">
      <sch:assert test="exists(f:quantity) or exists(f:rate)">mad-1: ДОЛЖНО содержать по крайней мере dosage.quantity или dosage.rate</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:OperationDefinition/f:parameter">
      <sch:assert test="exists(f:type) or exists(f:part)">opd-1: Необходимо указывать либо тип, либо parts</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:SupplyRequest/f:meta/f:security">
      <sch:assert test="not (exists(f:valueSet) and exists(f:code)) or exists(f:system)">cod-1: Если указан valueSet, то также необходимо указать и URI системы</sch:assert>
    </sch:rule>
    <sch:rule context="f:SupplyRequest/f:meta/f:tag">
      <sch:assert test="not (exists(f:valueSet) and exists(f:code)) or exists(f:system)">cod-1: Если указан valueSet, то также необходимо указать и URI системы</sch:assert>
    </sch:rule>
    ...
    <sch:rule context="f:SupplyRequest/f:contained/f:meta/f:security">
      <sch:assert test="not (exists(f:valueSet) and exists(f:code)) or exists(f:system)">cod-1: Если указан valueSet, то также необходимо указать и URI системы</sch:assert>
    </sch:rule>
    <sch:rule context="f:SupplyRequest/f:contained/f:meta/f:tag">
      <sch:assert test="not (exists(f:valueSet) and exists(f:code)) or exists(f:system)">cod-1: Если указан valueSet, то также необходимо указать и URI системы</sch:assert>
    </sch:rule>
    <sch:rule context="f:SupplyRequest/f:contained/f:meta/f:security">
      <sch:assert test="not (exists(f:valueSet) and exists(f:code)) or exists(f:system)">cod-1: Если указан valueSet, то также необходимо указать и URI системы</sch:assert>
    ...
    <sch:rule context="f:SupplyRequest/f:kind">
      <sch:assert test="count(f:coding[f:primary/@value='true'])&lt;=1">ccc-2: Только один кодинг из набора может быть выбран непосредственно пользователем</sch:assert>
    </sch:rule>
    <sch:rule context="f:SupplyRequest/f:kind/f:coding">
      <sch:assert test="not (exists(f:valueSet) and exists(f:code)) or exists(f:system)">cod-1: Если указан valueSet, то также необходимо указать и URI системы</sch:assert>
    </sch:rule>
    <sch:rule context="f:SupplyRequest/f:identifier/f:type">
      <sch:assert test="count(f:coding[f:primary/@value='true'])&lt;=1">ccc-2: Только один кодинг из набора может быть выбран непосредственно пользователем</sch:assert>
    </sch:rule>
    <sch:rule context="f:SupplyRequest/f:identifier/f:type/f:coding">
      <sch:assert test="not (exists(f:valueSet) and exists(f:code)) or exists(f:system)">cod-1: Если указан valueSet, то также необходимо указать и URI системы</sch:assert>
    </sch:rule>
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Patient/f:contact">
      <sch:assert test="f:name or f:telecom or f:address or f:organization">pat-1: ДОЛЖЕН содержать как минимум либо контактные данные, либо ссылку на организацию</sch:assert>
    </sch:rule>
